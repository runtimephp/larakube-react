...
export default function OrganizationManager() {
    ...
    return (
        <DropdownMenu open={open} onOpenChange={setOpen}>
            ...
            <DropdownMenuContent className="w-(--radix-dropdown-menu-trigger-width) min-w-56 rounded-lg p-0" align="start" side="bottom">
                <Command>
                    ...
                    <CommandList>
                        ...
                        <CommandGroup heading="Switch organization">
                            {organizations?.map((organization) => (
                                <CommandItem key={organization.id}>
                                    <div
                                    ...
                                    >
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <div className="max-w-30 truncate">{organization.name}</div>
                                        </div>
                                        <div className="item">{organization.slug === currentOrganization?.slug ? <Check /> : null}</div>
                                    </div>
                                </CommandItem>
                            ))}
                        </CommandGroup>
                        ...
                        <CommandGroup heading="Manage organization">
                            <CommandItem asChild className="cursor-pointer">
                                <Link href={route('cloud-accounts.index', { organization: currentOrganization?.slug })}>
                                    <Settings />
                                    Organization Settings
                                </Link>
                            </CommandItem>
                            <CommandItem className="cursor-pointer" onSelect={handleCreateOrganization}>
